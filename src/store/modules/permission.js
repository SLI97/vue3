			...
			return new Promise(resolve => {
				...
				// getRouters().then(res => {
					const res = {
						...
						// 	"hidden": false,
						// 	"component": "Layout",
						// 	"meta": {"title": "97官网", "icon": "guide"}
						// }
						]
					}
					const accessedRoutes = convertAsyncRouter(res.data)
...
function convertAsyncRouter(asyncRouterMap) {
	return asyncRouterMap.map(route => {
		...
		if (route.children != null && route.children && route.children.length) {
			route.children = convertAsyncRouter(route.children)
		}
		return route
	})
}
